...
@Named("refFilePermissionBean")
@ViewScoped
public class RefFilePermissionBean extends GenericCRUDBean<RefFilePermission>
	...
	private List<RefAgency> agencyList;
	...
	private Map<Integer,RefFileProperty> allFilesFoldersMap;
	...
	public void init(){
		...
		agencyList = service.getAgencyList(false);
	...
	public void initDisplay(){
		...
		initOrderedFolderFilesList();
	}
	
	private void updateHeader(){
		LOGGER.info("updateHeader()");
		
		PermHeader fileFolder = new PermHeader("Files/Folders",2,1);
		
		fheader = new ArrayList<PermHeader>();
		fheader.add(fileFolder);
...
//		header1.add(fileFolder);
		for(RefAgency ra : agencyList){
			PermHeader agency = new PermHeader(ra.getAgencyShortname());
			agency.setColSpan(ra.getRefAgencyGroupList().size());
			agency.setPermId(ra.getAgencyId());
			header1.add(agency);
			for(RefAgencyGroup rag : ra.getRefAgencyGroupList()){
				PermHeader agencyGroup = new PermHeader(rag.getAgencyGroupShortname(), 1, 1);
				agencyGroup.setPermId(rag.getAgencyGroupId());
				header2.add(agencyGroup);
			}
		}
//		for(RefAgency ra : agencyList){
//			PermHeader agency = new PermHeader(ra.getAgencyShortname());
...
//		}
	...
	public void updateFileFolderView(){
		...
		allFilesFoldersMap = new HashMap<Integer, RefFileProperty>();
		
		for(RefFileProperty rfp : allFolders){
			allFilesFoldersMap.put(rfp.getFilePropertyId(), rfp);
		}
	...
	private void updateFileAndFolderLevelSelectAll() {
		...
		for (Integer folderId : folderAndCorrespondingFilesMap.keySet()) {
			...
			//check by agency
			isChecked = true;
			checkedMainFolder = true;
			for (RefAgencyGroup agencyGroup : agencyGroupList) {
				for (Integer fileId : folderAndCorrespondingFilesMap.get(folderId)) {
					key = agencyGroup.getAgencyGroupId().toString().concat(FILE_MAP_SEPARATOR).concat(fileId.toString());
					LOGGER.info("key: "+ key);
					isChecked = displayDataMap.get(key) == null	|| displayDataMap.get(key) == false ? false : true;
					if (!isChecked) {
						checkedMainFolder = false;
						LOGGER.info("here");
						displayDataMap.put("ALL_AGENCY_" + agencyGroup.getAgencyGroupId() + FILE_MAP_SEPARATOR + folderId, false);
						displayDataMap.put("AGENCY_" + agencyGroup.getAgencyGroupId() + FILE_MAP_SEPARATOR + folderId, false);
						break;
					} else {
						LOGGER.info("here2");
						displayDataMap.put("ALL_AGENCY_" + agencyGroup.getAgencyGroupId() + FILE_MAP_SEPARATOR + folderId, true);
						displayDataMap.put("AGENCY_" + agencyGroup.getAgencyGroupId() + FILE_MAP_SEPARATOR + folderId, true);
					}
				}
			}
	...
	public void toggleAllFilesForAgency(Integer folderId,Integer agencyId){
		LOGGER.info("toggleAllFilesForAgency()" + agencyId + ":" + folderId);	
		
		boolean folderStat = displayDataMap.get("AGENCY_" + agencyId + FILE_MAP_SEPARATOR + folderId);
		
		if(displayDataMap.get("ALL_AGENCY_" + agencyId + FILE_MAP_SEPARATOR + folderId) != null && 
				displayDataMap.get("ALL_AGENCY_" + agencyId + FILE_MAP_SEPARATOR + folderId)){
			folderStat = false;
		}
		LOGGER.info("folderStat: " + folderStat);
		
		for(Integer fileId : folderAndCorrespondingFilesMap.get(folderId)){
			String key = agencyId.toString().concat(FILE_MAP_SEPARATOR).concat(fileId.toString());
			LOGGER.info("KKK: " + key);
...
//			checkAllAgencyPerFolder(key);
	...
	private void initOrderedFolderFilesList(){
		LOGGER.info("initOrderedFolderFilesList()");
		orderedFolderFiles = new ArrayList<RefFileProperty>();
		for(Integer folderId : folderAndCorrespondingFilesMap.keySet()){
			RefFileProperty folder = allFilesFoldersMap.get(folderId);
			
			if(folder != null){
				orderedFolderFiles.add(folder);
				for(Integer fileId : folderAndCorrespondingFilesMap.get(folderId)){
					RefFileProperty file = allFilesFoldersMap.get(fileId);
					orderedFolderFiles.add(file);
				}
			}else{
				LOGGER.info("Folder not found: " + folderId);
			}
	...
	private List<RefFileProperty> orderedFolderFiles;
...
//								fileHeader.setPermId(focusFileRfp.getFilePropertyId());
	...
	public String formatAgencyAndFolder(int agencyId,int folderId){
		StringBuilder sb = new StringBuilder();
		sb.append("'");
		sb.append(agencyId);
		sb.append(FOLDER_MAP_SEPARATOR);
		sb.append(folderId);
		sb.append("'");
		LOGGER.info("eto: " + sb.toString());
		return sb.toString();
	}
...
//		}
		...
		if(ownerAgency == null || targetAgency == null){
			LOGGER.severe("OwnerAgency or TargetAgency is null");
			return;
		}
	...
	public List<RefAgency> getAgencyList() {
		return agencyList;
	}

	public void setAgencyList(List<RefAgency> agencyList) {
		this.agencyList = agencyList;
	}

	public Map<Integer,RefFileProperty> getAllFilesFoldersMap() {
		return allFilesFoldersMap;
	}

	public void setAllFilesFoldersMap(Map<Integer,RefFileProperty> allFilesFoldersMap) {
		this.allFilesFoldersMap = allFilesFoldersMap;
	}

	public List<RefFileProperty> getOrderedFolderFiles() {
		return orderedFolderFiles;
	}

	public void setOrderedFolderFiles(List<RefFileProperty> orderedFolderFiles) {
		this.orderedFolderFiles = orderedFolderFiles;
	}
}
